...
class OptionsMenu extends MusicBeatState
{
	...
	var options:Array<OptionCatagory> = [
		...
		new OptionCatagory("Misc", [
			...
			new GrilledCheeseOption("Do you know how to make a Grilled cheese?")
		])
	];
		...
		if (acceptInput)
		{
			...
			if (isCat)
			{
				...
				if (currentSelectedCat.getOptions()[curSelected].getAccept())
					...
					versionShit.text = "Offset (Left, Right, Shift for slow): " + PlayState.truncateFloat(FlxG.save.data.offset,2) + " - Description - " + currentDescription;
			}
			else
			{
				if (FlxG.keys.pressed.SHIFT)
